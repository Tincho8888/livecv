...
export default function EditorPage() {
  ...
  async function loadCVs(userId) {
    setCvsLoading(true);
    try {
      setCvList((await fetchCVs(userId)) || []);
    } catch (e) {
      console.error(e);
    } finally {
      setCvsLoading(false);
  ...
  async function handleImportFile(file) {
    ...
    try {
      ...
      console.error(e);
      alert("Error al importar el CV. Intentá de nuevo.");
    } finally {
      setImporting(false);
    }
  }
  ...
  async function handleTogglePublic(val) {
    ...
    if (currentCvId) {
      try {
        await updateCVMeta(currentCvId, { is_public: val });
      } catch {}
    }
  }
  ...
  async function handleToggleTheme(theme) {
    ...
    if (currentCvId) {
      try {
        await updateCVMeta(currentCvId, { default_theme: theme });
      } catch {}
    }
  }

  // ── Loading ───────────────────────────────────────────────
  if (session === undefined || importing)
    return (
      <div
        style={{
          minHeight: "100vh",
  ...
  return (
    <div className="app">
      <Nav
        ...
        onLogoClick={() => setScreen("dashboard")}
      ...
      {screen === "editor" && (
        <div className="editor-layout">
          ...
          <div className="form-panel">
            <EditorForm active={activeSection} data={data} session={session} onUpdate={handleDataChange} />
          </div>
